...
import { query, sql, all } from '@/lib/db/sql';
...
export async function bookSlotWithin(
  ...
  //
  // The optional condition goes through `all()`, which drops it when there is
  // no sessionKey rather than leaving a dangling AND.
  //
  // This query once failed for every website booking and no test caught it,
  // because the branch fired only when a sessionKey was supplied and no test
  // passed one. The regression tests for that now live in tests/db.sql.test.ts
  // and cover both the present and the absent case. (The mechanism recorded at
  // the time — nested fragments being bound as parameters — does not reproduce
  // on the current Prisma; see the note in lib/db/sql.ts.)
  const heldRows = await query<{ jobs: number; units: number | null; minutes: number | null }>(
    sql`
      SELECT COUNT(*) AS "jobs", SUM("units") AS "units", SUM("minutes") AS "minutes"
        FROM "quota_holds"
       WHERE ${all([
         sql`"quotaDayId" = ${bucket.id}`,
         sql`"expiresAt" > NOW()`,
         sessionKey ? sql`"sessionKey" <> ${sessionKey}` : null,
       ])}
    `,
    { name: 'liveHoldsForBucket', tx },
  );
  ...
  // SUM over no rows is SQL NULL, not zero — the ?? is load-bearing.
  const heldJobs = heldRows[0]?.jobs ?? 0;
  const heldUnits = heldRows[0]?.units ?? 0;
  const heldMinutes = heldRows[0]?.minutes ?? 0;
