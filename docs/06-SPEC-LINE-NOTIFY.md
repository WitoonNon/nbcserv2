...
### ⛔ ติดตั้งแต่ขั้นแรก — ตรวจแล้วเมื่อ 18 ส.ค. 2569

**@nbcservice ยังไม่ได้เปิด Messaging API เลย**
(`manager.line.biz` → ตั้งค่า → Messaging API → สถานะ *ไม่ใช้งาน*)

ตราบใดที่ยังไม่เปิด จะ**ไม่มี Channel ให้เห็นใน LINE Developers Console**
และไม่มีทั้ง Channel ID, Secret, Access Token หรือแท็บ Webhook ให้ตั้งค่า

**ลูกค้าหรือแอดมินของ OA ต้องเป็นคนกดเปิดเอง** — เราทำแทนไม่ได้
ถ้าไม่ได้เป็นแอดมิน

| | หาได้ที่ไหน (หลังเปิด Messaging API แล้ว) |
|---|---|
| `LINE_CHANNEL_ID` | LINE Developers → Channel ของ @nbcservice → Basic settings |
| `LINE_CHANNEL_SECRET` | Channel เดียวกัน → Basic settings |
| `LINE_CHANNEL_ACCESS_TOKEN` | Channel เดียวกัน → Messaging API → ออกแบบ long-lived |
| สิทธิ์ตั้งค่า Webhook URL | ต้องเปิด "Use webhook" และใส่ URL ของเรา |
| webhook เดิมที่อาจมีอยู่ | ตรวจก่อนทับ — ถ้ามีระบบอื่นใช้อยู่ การทับจะทำให้ระบบนั้นพัง |
